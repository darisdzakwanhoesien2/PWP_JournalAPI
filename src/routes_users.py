...
def _build_user_response(user):
    ...
    user_dict['_links'] = utils.user_links(user.id)
    return user_dict
...
def get_users():
    ...
    """
    users_data = data_store.load_users()
    users = [models_orm.User.from_dict(u) for u in users_data]
    result = [_build_user_response(user) for user in users]
    response = {
        'items': result,
        '_links': utils.users_collection_links()
    }
...
def register_user():
    ...
    new_id = data_store.get_next_id(users_data)
    user = models_orm.User(id=new_id, username=data['username'], email=data['email'])
    users_data.append(user.to_dict())
    data_store.save_users(users_data)
...
def get_user(user_id):
    ...
    user = models_orm.User.from_dict(user_data)
    user_dict = _build_user_response(user)
...
def update_user(user_id):
    ...
    if 'username' in data:
        ...
        for u in users_data:
            if u['username'] == data['username'] and u['id'] != user_id:
                abort(400, description='Username already exists')
    ...
    data_store.save_users(users_data)
    user = models_orm.User.from_dict(users_data[user_index])
    user_dict = _build_user_response(user)
    return jsonify(user_dict), 200
